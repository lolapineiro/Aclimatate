...
const resulBusquedasElement = document.getElementById('busquedasanteriores');
let mapacargado = 0;
...
buttonSearch.addEventListener( 'click', event=> {
  ...
  .then((json) => {
        const busquedas = JSON.parse(localStorage.getItem('busquedas')) || [];
          ...
          `<h3>${json.name}</h3>
            <ul>
            <li class="list-group-item"><img src="http://openweathermap.org/img/wn/${json.weather[0].icon}.png"/></li>
            ...
            resulElement.innerHTML = info; 
            busquedas.push(info)
            localStorage.setItem('busquedas', JSON.stringify(busquedas));
            
            if(mapacargado==0){
              var map = tt.map({
                key: 'fG60F5jgaAXbMfWdcig5bmrLsjZUFOCw',
                container: 'map',
                dragPan: !isMobileOrTablet(),
                center: [json.coord.lon, json.coord.lat],
                zoom: 5
              });
...
if(busquedas.length > 0){
  ...
  for(let i=busquedas.length-1;i>=0;i--){
    resulBusquedasElement.innerHTML += '<div class="frosted">' +  busquedas[i] + '</div>'; 
